  ...
  public:
	  ...
	  cv::Mat ransacTest(const std::vector<cv::DMatch>& matches,
		...
		// The F matrix will be recomputed with all accepted matches
			...
			std::cout << points1.size() << "  -----   " <<  points2.size()  << endl;
			...
			if (refineM) {
				...
				std::cout << fundamental.rows << "  -----   " << fundamental.cols << endl;
				// refine the matches
	  ...
	  cv::Mat match(cv::Mat& image1, cv::Mat& image2, // input images 
		...
		cv::Mat imMatches;
		cv::drawMatches(image1, keypoints1, image2, keypoints2, outputMatches, imMatches, cv::Scalar(0, 0, 255), cv::Scalar::all(-1));
        //cv::imshow("matches_window", imMatches);
        cv::namedWindow("image_robust_matcher", cv::WINDOW_AUTOSIZE);
        cv::imshow("image_robust_matcher", imMatches);
		if (outputMatches.size() > 20)
			cv::imwrite("/home/bigby/ws/catkin_ws/result_potential_match.bmp",imMatches );
