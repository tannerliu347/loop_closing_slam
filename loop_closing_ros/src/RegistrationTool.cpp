...
bool RegistrationTool::Registration2d(RegistrationData2d& data){
    ...
    data.F = robust_matcher.match(data.img_candidate,data.img_cur,data.matches,data.points_cur,data.points_candidate,data.descriptors_candidate,data.descriptors_cur);
    if (data.F.rows != 3){
        return false;
    }
